...
# ========================================
# may 2026 v1.5                  torchk.sh
#
# For use with http://TorRouter.nl
#
# This script is for TorRouter devices: Fritz!box 4040, Linksys WHW03 v2, vmware x86_64.
# Program uses apk (instead of opkg), crontabs, Tor and curl to grab 'https://check.torproject.org'.
# It is used to check good working Tor every hour with crontab and changes hourly crontab
# to check every 5 minutes if Tor is failed, back to hourly if Tor is ok and adjust LEDs accordingly.
...
if [ ! "$1" = "" ]; then
  ...
  if [ -e /etc/crontabs/root ]; then cat /etc/crontabs/root | grep -v /etc/tor/torchk.sh > /tmp/root.tmp;
    ...
    if [ "$1" = "error" ]; then echo "*/5 * * * * /etc/tor/torchk.sh" >> /tmp/root.tmp; fi
